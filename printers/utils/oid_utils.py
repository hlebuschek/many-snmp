...
def load_oid_mapping(file_path):
    ...
    try:
        with open(file_path, "r") as file:
            for line in file:
                if line.strip() and not line.startswith(
                    ...
                    if (
                        len(parts) >= 4
                    ):  # Проверяем, что строка содержит OID, производителя, тип и модель
                        oid = parts[0]
        ...
        logger.info(
            f"Справочник OID успешно загружен. Загружено {len(oid_to_model)} записей."
        )
    ...
    for oid, value in snmp_data.items():
        ...
        if "mib-2.3.1.1.2.14.1" in oid:
            if isinstance(value, str) and value.startswith(
                "0x"
            ):  # Проверяем, что значение в шестнадцатеричном формате
                try:
